extends "res://entities/units/consumables/consumable.gd"
# =============================================================================
...
# fallback applies a raw heal so value is never lost.
func _replay_heal(player) -> void:
	if Config.EFFECT_APPLY_METHOD != "" and has_method(Config.EFFECT_APPLY_METHOD):
		call(Config.EFFECT_APPLY_METHOD, player)
		return
	if player == null:
		return
	if not player.has_method(Config.PLAYER_HEAL_METHOD):
		return  # fail safe: skip rather than crash on a wrong method name
	player.call(Config.PLAYER_HEAL_METHOD, get_base_heal())
