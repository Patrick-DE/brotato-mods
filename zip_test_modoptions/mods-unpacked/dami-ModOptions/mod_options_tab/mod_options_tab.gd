	...
	for config_key in mod_config.keys():
		...
		if config_key.begins_with("enum_"):
			...
			if config_key.ends_with("_options"):
				var enum_value = mod_config[config_key.trim_suffix("_options")]
...
func _init_drop_down(parent: Node, mod_name: String, config_key: String, config_value, config_options: Array):
	...
	new_drop_down.connect("item_selected", self, "signal_option_button_item_selected", [config_key.trim_prefix("enum_").trim_suffix("_options"), config_options, mod_name])
